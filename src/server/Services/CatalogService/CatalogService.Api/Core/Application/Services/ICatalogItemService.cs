...
namespace CatalogService.Api.Core.Application.Services
{
    ...
    {
        Task<ResponseDto<PaginatedItemsViewModel<CatalogItem>>> GetItemsAsync(int pageIndex = 0, int pageSize = 10, string ids = null);
        Task<ResponseDto<CatalogItem>> GetItemByIdAsync(int id);
        Task<ResponseDto<PaginatedItemsViewModel<CatalogItem>>> ItemsWithNameAsync(string name, int pageSize = 10, int pageIndex = 0);
        Task<ResponseDto<PaginatedItemsViewModel<CatalogItem>>> ItemsByTypeIdAndBrandIdAsync(int? catalogTypeId, int? catalogBrandId, int pageSize = 10, int pageIndex = 0);
